/*
 * THIS OBJECT IS OBSOLETE AND WILL BE REMOVED IN DUE TIME!
 * ONE SHOULD NOT USE IT. RATHER USE THE FUNCTION more() THAT
 * IS DEFINED IN PLAYERS.
 *
 * /obj/more.c
 *
 ...
 * someone to read something (MORE_OBJECT has been defined in files.h):
 *
 * seteuid(getuid());
 * clone_object(MORE_OBJECT)->more(args);
 *
 ...
 *     clone_object(MORE_OBJECT)->more(string filename, int start_line);
 *
 ...
 *     clone_object(MORE_OBJECT)->more(arg1, arg2, string return_function);
 *
 * WARNING: The function more() operates on this_player().
 *
 * Version 2.0 by /Mercade, May 24 1994.
 */
...
private string  ret_func; /* the function to call when finished reading     */
...
"   a(ll)        display all the text remaining\n" +
...
"   q(uit)       quit reading document (x or exit idem)\n\n");
	...
	if (filename)
	{
	    cat(filename, lineno + 1);
	    write("EOF\n");
	}
	else
	{
	    write(implode(lines[lineno..], "\n"));
	}
	done();
	return 1;
